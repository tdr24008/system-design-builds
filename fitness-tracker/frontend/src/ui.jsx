...
export const Card = ({ children, style = {}, padded = true, onClick }) => (
...
export const Stat = ({ value, unit, size = 28, color = T.text, weight = 600 }) => (
  <div style={{
...
export const ActivityDisc = ({ type, size = 36, active = false }) => {
...
export const WeeklyBars = ({ data, height = 60, todayIdx = 5 }) => {
...
export const Sparkline = ({ data, width = 80, height = 24, color = T.orange }) => {
...
export const Segmented = ({ options, value, onChange, accent = T.orange, size = 'md' }) => {
...
export const MiniRoute = ({ width = 80, height = 60, seed = 1 }) => {
...
export const RouteBanner = ({ height = 200 }) => (
  <div style={{
...
export const HRChart = ({ width = 358, height = 120 }) => {
  const N = 60;
  const data = [];
  for (let i = 0; i < N; i++) {
    const t = i / (N - 1);
    ...
    if (t > 0.55 && t < 0.7) v += 12;
    if (t > 0.85) v -= 30 * (t - 0.85) * 6.6;
